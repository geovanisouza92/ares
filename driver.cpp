...
bool Driver::parse_file(const string& filename) {
	...
	if (fs::exists(file) && file.extension() == string("." LANG_EXTENSION)) {
		...
		warning(
				war_tail "This isn't seems a valid " LANG_NAME " file: "
						+ filename);
	return false;
}
...
string Driver::resume_messages() {
	stringstream result;
	if (errors > 0 || warnings > 0 || hints > 0) {
		result << "Messages: ";
		if (verbose_mode >= VerboseMode::ErrorsOnly && errors > 0)
			result << COLOR_BRED << errors
					<< COLOR_RESET << (errors > 1 ? " errors " : " error ");
		if (verbose_mode >= VerboseMode::ErrorsAndWarnings && warnings > 0)
			result
					<< COLOR_BPURPLE << warnings
					<< COLOR_RESET << (warnings > 1 ? " warnings " : " warning ");
		if (verbose_mode >= VerboseMode::ErrorsWarningsAndHints && hints > 0)
			result << COLOR_BBLUE << hints
					<< COLOR_RESET << (hints > 1 ? " hints " : " hint ");
		result << endl;
	}
	return result.str();
}

void Driver::make_things_happen(FinallyAction::Action action, ostream& out) {
	switch (action) {
