...
static inline string trim(string str, char c) {
...
if (driver.verbose_mode >= VerboseMode::ErrorsOnly) { \
    ...
    if (driver.total_lines > 0) \
        cout << LANG_NAMESPACE::Util::resume_statistics(files_processed, driver.total_lines, x); \
}
...
int main(int argc, char** argv) {
	...
	po::options_description desc(
			"Usage: " LANG_SHELL_NAME " [Options] files\n\nOptions");
	desc.add_options()("check-only,c", "Enable check only")("eval,e",
			po::value<string>(), "Evaluate and compile <arg>")("help,h",
			"Print this message and exit")("input-file",
			po::value<vector<string> >(),
			"[Optional] Use <arg> as input file(s).")("verbose",
			po::value<int>()->default_value(0), "Set the verbose mode [0..3]")(
	...
	if (options.count("eval")) {
		...
		if (!line.empty()) {
			...
			if (parse_result) {
				if (driver.errors > max_errors)
					return 1;
				driver.make_things_happen(
						(driver.check_only ?
								FinallyAction::None :
								FinallyAction::PrintResults), cout);
			}
			STATS(false)
		}
	}
	...
	if (options.count("input-file")) {
		...
		for (vector<string>::iterator file = input_files.begin();
	...
	if (driver.verbose_mode >= VerboseMode::ErrorsWarningsAndHints
			&& !messages.empty())
		for (vector<string>::iterator message = messages.begin();
	...
	if (!files.empty()) {
		...
		for (vector<string>::iterator file = files.begin(); file < files.end();
				file++) {
